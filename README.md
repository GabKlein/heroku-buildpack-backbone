...
# Heroku Buildpack for Backbone Applications

This buildpack will work out of the box with Backbone generated applications. It installs Node, Nginx and generates a production build with the Backbone.

## Usage

Creating a new Heroku instance from an Backbone application's parent directory:

    $ heroku create --buildpack https://github.com/GabKlein/heroku-buildpack-backbone.git
...
For most Backbone applications that make any kind of authenticated requests HTTPS should be used. It supports the headers `X-Forwarded-Proto` ([used by Heroku](https://devcenter.heroku.com/articles/http-routing#heroku-headers)) and `CF-Visitor` ([used by CloudFlare](https://support.cloudflare.com/hc/en-us/articles/200170536-How-do-I-redirect-HTTPS-traffic-with-Flexible-SSL-and-Apache-)). Enable this feature in Nginx by setting `FORCE_HTTPS`:
...
Choose the environment you want to build by setting:
    ...
    $ heroku config:set BACKBONE_ENV=production

### Before and After Hooks

Have the buildpack run your own scripts before and after the `backbone build` by creating a `hooks/before_hook.sh` or `hooks/after_hook.sh` file in your Backbone application:
...
Sometimes it is necessary to rebuild NPM modules or Bower dependencies from scratch.  This can become necessary when updating Backbone or Backbone midway through a project and cleaning the Bower and NPM caches doesn't always refresh the cache in the Dyno during the next deployment.  In those cases, here is a simple and clean way to force a rebuild.
...
In your Backbone application, add a `config/nginx.conf.erb` file and add your own Nginx configuration.

*You should copy the existing configuration file in this repo and make changes to it for best results.*

### Caching

The Backbone buildpack caches your NPM and Bower dependencies by default. This is similar to the [Heroku Buildpack for Node.js](https://github.com/heroku/heroku-buildpack-nodejs). This makes typical deployments much faster. Note that dependencies like [`components/backbone#canary`](http://www.backbone.com/#using-canary-build-instead-of-release) will not be updated on each deploy.
...
Be sure to save any Bower or NPM resolutions. Now, let's build your Backbone application locally:
